...
const ViewPhotography = () => {
  ...
  const [editablePhotography, setEditablePhotography] = useState({});

  useEffect(() => {
    if (data) {
      setPhotography(data);
      setEditablePhotography({ ...data });
    }
  }, [data]);

  const handleChange = (e) => {
    setEditablePhotography((prev) => ({
      ...prev,
  ...
  const handleSave = async () => {
    try {
      ...
      setPhotography(editablePhotography);
  ...
  if (!photography) return <p className="text-center text-gray-600">Loading...</p>;
  ...
  return (
    <div className="max-w-3xl mx-auto p-4">
      ...
      <div className="relative flex justify-center mb-4">
        {editablePhotography?.photos?.length > 0 ? (
          <div className="relative">
            <img
              src={editablePhotography.photos[0]}
              alt="photography"
      ...
      <div className="bg-white shadow-md rounded-lg p-6 space-y-4">
        ...
        <div>
          ...
          <input
            type="text"
            name="title"
            value={editablePhotography?.title || ""}
        ...
        <div>
          ...
          <textarea
            name="desc"
            value={editablePhotography?.desc || ""}
            onChange={handleChange}
            rows="4"
            className="w-full p-2 border rounded focus:ring focus:ring-green-300 outline-none"
          ></textarea>
