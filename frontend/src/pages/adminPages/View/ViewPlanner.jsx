...
const ViewPlanner = () => {
  ...
  const [editablePlanner, setEditablePlanner] = useState({});

  useEffect(() => {
    if (data) {
      setPlanner(data);
      setEditablePlanner({ ...data });
    }
  }, [data]);

  const handleChange = (e) => {
    setEditablePlanner((prev) => ({
      ...prev,
  ...
  if (!planner) return <p className="text-center text-gray-600">Loading...</p>;

  return (
    <div className="max-w-3xl mx-auto p-4">
      <h2 className="text-2xl font-semibold text-gray-700 text-center mb-4">
        Edit planner Service
      </h2>

      {/* Image Display */}
      <div className="relative flex justify-center mb-4">
        {editablePlanner?.photos?.length > 0 ? (
          <div className="relative">
            <img
              src={editablePlanner.photos[0]}
              alt="planner"
