...
const ViewDestination = () => {
  ...
  useEffect(() => {
    if (data) {
      setDestination(data);
      setEditableDestination({ ...data });
    }
  }, [data]);
  const handleChange = (e) => {
    setEditableDestination((prev) => ({
      ...prev,
  ...
  if (!destination) return <p className="text-center text-gray-600">Loading...</p>;

  return (
    <div className="max-w-3xl mx-auto p-4">
      <h2 className="text-2xl font-semibold text-gray-700 text-center mb-4">
        Edit destination Service
      </h2>

      {/* Image Display */}
      <div className="relative flex justify-center mb-4">
        {editableDestination?.photos?.length > 0 ? (
          <div className="relative">
            <img
              src={editableDestination.photos[0]}
              alt="destination"
      ...
      <div className="bg-white shadow-md rounded-lg p-6 space-y-4">
        ...
        <div>
          ...
          <input
            type="text"
            name="title"
            value={editableDestination?.title || ""}
        ...
        <div>
          ...
          <textarea
            name="desc"
            value={editableDestination?.desc || ""}
            onChange={handleChange}
            rows="4"
            className="w-full p-2 border rounded focus:ring focus:ring-green-300 outline-none"
          ></textarea>
